...
A host's risk is derived from the set of its open TCP ports:
    ...
    CRÍTICO  → at least one port in _CRITICAL  (FTP/Telnet/SMB/RDP)
    MEDIO    → at least one port in _MEDIUM   (HTTP/DNS/SSDP/NFS)
    BAJO     → has open ports but none of the above
    MÍNIMO   → no open ports (closed host)
...
def classify_risk(ports: list[str]) -> str:
    ...
    *ports* is the list returned by sentinel.deep_scan: each entry is a
    ...
    Possible return values: MÍNIMO / BAJO / MEDIO / CRÍTICO.
    """
